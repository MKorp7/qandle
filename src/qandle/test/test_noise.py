...
@pytest.mark.parametrize(
    "builder,state",
    [
        (lambda eps: PhaseFlip(p=eps, qubit=0), torch.eye(2, dtype=torch.float32)),
        (lambda eps: Depolarizing(p=eps, qubit=0), torch.eye(2, dtype=torch.float32)),
        (lambda eps: Dephasing(gamma=eps, qubit=0), torch.eye(2, dtype=torch.float32)),
        (lambda eps: AmplitudeDamping(gamma=eps, qubit=0), torch.eye(2, dtype=torch.float32)),
        (lambda eps: CorrelatedDepolarizing(p=eps, qubits=(0, 1)), torch.eye(4, dtype=torch.float32)),
    ],
)
def test_near_zero_channels_skip(builder, state):
    n = int(math.log2(state.shape[-1]))
    eps = NEAR_ZERO / 2
    ch = builder(eps).build(num_qubits=n)
    with patch.object(ch, "_local_kraus", side_effect=AssertionError("should not be called")) as spy:
        out = ch(state)
    spy.assert_not_called()
    assert out is state
...
@pytest.mark.parametrize(
    "builder,state",
    [
        (lambda eps: PhaseFlip(p=eps, qubit=0), torch.tensor([1.0, 0.0], dtype=torch.float32)),
        (lambda eps: Depolarizing(p=eps, qubit=0), torch.tensor([1.0, 0.0], dtype=torch.float32)),
        (lambda eps: Dephasing(gamma=eps, qubit=0), torch.tensor([1.0, 0.0], dtype=torch.float32)),
        (lambda eps: AmplitudeDamping(gamma=eps, qubit=0), torch.tensor([1.0, 0.0], dtype=torch.float32)),
        (
            lambda eps: CorrelatedDepolarizing(p=eps, qubits=(0, 1)),
            torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=torch.float32),
        ),
    ],
)
def test_near_zero_channels_skip_trajectory(builder, state):
    n = int(math.log2(state.shape[-1]))
    eps = NEAR_ZERO / 2
    ch = builder(eps).build(num_qubits=n)
    with patch.object(ch, "_local_kraus", side_effect=AssertionError("should not be called")) as spy:
