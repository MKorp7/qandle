...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    qubit_range = range(args.min_qubits, args.max_qubits + 1, args.step)

    run_id = 0
    try:
        for spec in iter_backends(device_keys):
            if not spec.is_available():
                reason = spec.unavailable_reason or "unknown"
                print(f"Skipping {spec.key}: {reason}")
                continue

            print(f"Running qubit scaling for {spec.key}")

            for n_qubits in qubit_range:
                circuit = utils.scaling_basic(n_qubits, args.depth, args.seed)
                instance = spec.make(
                    n_qubits=n_qubits,
                ...
                # Record a timezone-aware UTC timestamp (required to avoid the
                # datetime.utcnow deprecation warning) but keep the canonical
                # "Z" suffix used by existing benchmark tooling. Use
                # datetime.timezone.utc rather than datetime.UTC for
...
def main(argv: Iterable[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Qubit scaling benchmark")
    ...
    parser.add_argument("--reps", type=int, default=5, help="Timing repetitions (warm-up + reps)")
    parser.add_argument("--seed", type=int, default=123, help="Base RNG seed")
    parser.add_argument("--out", type=str, default="qubit_scaling.csv", help="Output CSV path")
    parser.add_argument("--baseline", type=str, default=None, help="Optional baseline key for downstream comparison")
    parser.add_argument("--tags", type=str, default="", help="Tag string to embed in the CSV")
