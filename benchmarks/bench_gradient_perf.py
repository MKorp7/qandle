...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    host = platform.node()
    tags = args.tags or ""
    circuit = utils.grad_basic(args.qubits, args.depth, args.seed)
    ...
    try:
        for spec in iter_backends(device_keys):
            ...
            if not spec.features.get("supports_grad", False):
                print(f"Skipping gradient run for {spec.key}: no_grad_support")
                base_row: Dict[str, str] = {
                    ...
                    "dtype": spec.defaults.get("dtype", ""),
                    "threads": str(args.threads or spec.defaults.get("threads", "")),
                    "bond_dim": str(args.bond_dim or spec.defaults.get("bond_dim", "")),
                    "splitting": utils.format_bool_flag(args.splitting),
                    "shots": str(utils.shots_value(args.shots)),
                    ...
                    "seed": str(args.seed),
                    "run_id": str(run_id),
                    "execution_time_s": "0.000000",
                    "execution_time_std_s": "0.000000",
                    "reps": str(args.reps),
                    "peak_cpu_mb": "0.000",
                    "peak_gpu_mb": "0.000",
            ...
            print(f"Running gradient benchmark for {spec.key}")
            ...
            try:
                gradient, grad_norm, mean_time, std_time, peak_cpu, peak_gpu = utils.execute_parametric_circuit(
                    circuit,
                    dtype,
                    spec.hardware,
                    args.reps,
...
def main(argv: Iterable[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Gradient performance benchmark")
    ...
    parser.add_argument("--out", type=str, default="gradient_perf.csv", help="Output CSV path")
    ...
    parser.add_argument("--depth", type=int, default=4, help="Circuit depth")
